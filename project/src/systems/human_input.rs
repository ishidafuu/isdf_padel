...
pub fn human_input_system(
    ...
    for (human, mut input_state) in query.iter_mut() {
        ...
        // ジャンプ入力（B キー）
        input_state.jump_pressed = keyboard.just_pressed(KeyCode::KeyB);

        // ショット入力（V キー）
        let shot_key_pressed = keyboard.pressed(KeyCode::KeyV);
        let shot_key_just_pressed = keyboard.just_pressed(KeyCode::KeyV);
