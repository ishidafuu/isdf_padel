...
- 後端（Leftコート側）: X = `-config.Court.Depth / 2` (デフォルト: -3.0m)
- 後端（Rightコート側）: X = `+config.Court.Depth / 2` (デフォルト: +3.0m)
...
- ネットX座標: X = 0（コート中央、Left/Rightの境界）
...
**THE SYSTEM SHALL** コートをLeftコートとRightコートに区分する
- Leftコート範囲: X < 0（ネットより手前、画面左側）
- Rightコート範囲: X >= 0（ネット含む、画面右側）
...
- サービスライン位置（Left側）: X = `-config.Court.ServiceBoxDepth` (デフォルト: -1.5m)
- サービスライン位置（Right側）: X = `+config.Court.ServiceBoxDepth` (デフォルト: +1.5m)
...
### 2026-01-09 - v2.2.0（CourtSide enum リネーム）

- **REQ-30501-001**: 1Pコート → Leftコート、2Pコート → Rightコートに変更
- **REQ-30501-003**: ベースライン表記を Left/Right に統一
- **REQ-30501-005**: ネット境界表記を Left/Right に変更
- **REQ-30501-006**: コート区分表記を 1P/2P から Left/Right に変更
- **REQ-30501-008**: サービスライン表記を Left/Right に変更
- **備考**: 座標系の方向説明を Right に統一
...
- **REQ-30501-006**: コート区分をX軸基準に修正（Left/Right）
