...
| 経過時間 | 威力係数 | 安定性係数 | 角度補正 | 説明 |
|---------|---------|-----------|---------|------|
| 0.0s | 1.0 | 0.6 | -5° | ライジング |
| 0.3s | 0.9 | 0.8 | 0° | 上がり中 |
| 0.5s | 0.8 | 1.0 | 0° | 頂点 |
| 0.8s | 0.6 | 0.9 | +10° | 落ち際 |
| 1.0s+ | 0.4 | 0.7 | +20° | 低くなる |
...
**WHEN** 全ての入力要素が確定する
**THE SYSTEM SHALL** 威力を計算する

```
power = base_power
      × input_power_factor      // 入力方式
      × height_power_factor     // 打点高さ
      × timing_power_factor     // タイミング
      × approach_power_factor   // 入り方
      × distance_power_factor   // 距離
```

**データ**: config.shot.base_power (デフォルト: 15.0 m/s)
