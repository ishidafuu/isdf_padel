...
**Player1の場合**:
```
target_x = lerp(net_x + margin, baseline_x - margin, (input.x + 1.0) / 2.0)
```
...
**WHEN** ショットを実行する
**AND** Y方向入力がある（W/Sキー）
**THE SYSTEM SHALL** 着地のZ座標（画面上下）を調整する

```
target_z = input.y * (court_width / 2 - margin)
```

| 入力Y（キー） | 着地位置 | 説明 |
|--------------|---------|------|
| -1.0（S） | 画面下側 | クロスorストレート |
| 0.0 | 中央 | センター |
| +1.0（W） | 画面上側 | クロスorストレート |
