...
| 機能 | 説明 |
|------|------|
| アウト判定 | コート外着地で失点（主要失点条件） |
| ツーバウンド判定 | 自コートで2回バウンドで失点 |
| ネットフォルト判定 | ネットを越えなかった場合の失点 |
...
| ファイル | REQ-ID範囲 | 要件数 |
|---------|-----------|-------|
| 30301_ai_movement_spec.md | REQ-30301-001 ~ 005 | 5 |
| 30302_ai_shot_spec.md | REQ-30302-001 ~ 005 | 5 |
...
| ファイル | REQ-ID範囲 | 要件数 |
|---------|-----------|-------|
| 30501_court_spec.md | REQ-30501-001 ~ 008 | 7 |
...
| ファイル | REQ-ID範囲 | 要件数 |
|---------|-----------|-------|
| 30901_point_judgment_spec.md | REQ-30901-001 ~ 005 | 5 |
...
- 「含まない機能」をv0.2以降からv0.4以降に変更
- 実装済み機能を削除（ショット属性、AI難易度、スピン等）
- ダッシュ、スタミナ削除（テニスでは不要）

### 2026-01-08 - v2.0.0（テニスへ変更）

- **ゲームプレイ**: 壁・天井反射 → アウト判定に変更
- **304_ball**: 壁・天井反射削除、アウト判定追加
- **305_court**: オープンコート化、壁・天井削除、アウト境界追加
- **309_referee**: アウト判定を主要失点条件として明記
- **データ定義**: Court境界をテニス用に調整（天井高さ削除）
