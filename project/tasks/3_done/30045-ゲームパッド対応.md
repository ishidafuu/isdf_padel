...
Player1をキーボードとゲームパッド両方で操作可能にする。
...
| スコープ | v0.4に追加 |
| 対応範囲 | 1コントローラー |
| 割当方式 | Player1=キーボード+ゲームパッド併用、Player2=AI |
...
- [x] Player1(device_id=0)がキーボード+ゲームパッド併用で操作可能
...
1. `gamepad_input.rs` - 新規作成（キーボード+ゲームパッド併用モード）
2. `mod.rs` - モジュール追加
3. `main.rs` - システム登録
