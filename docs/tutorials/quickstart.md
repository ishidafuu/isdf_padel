...
│  ─────────────────────────────────────────────────────────────  │
│  フォルダ構成・CLAUDE.md・GitHub設定を手動で作成                   │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
│  Phase 3: タスク管理 & 実装                                       │
│  ─────────────────────────────────────────────────────────────  │
│  📝 task-registration.md → 🗂️ task-manager-agent → 💻 impl-agent │
...
| アイコン | エージェント | 責務 | 入力 | 出力 |
|---------|-------------|------|------|------|
| 📝 | task-registration.md | プランファイル→タスクファイル変換 | プランファイル | Markdownタスクファイル |
...
**責務分離:**
- **task-registration.md**: プランファイル/仕様書 → タスクファイル変換専門
...
| バグ修正 | `B30XXX-NNN` | `project/tasks/` | ✅ | 既存機能のバグ修正 |
| リファクタ | `R30XXX-NNN` | `project/tasks/` | ✅ | 既存機能の改善・最適化 |
...
```
人間: 「プレイヤーのジャンプ機能の仕様を作りたい」

メイン Claude Code の動作（spec-agent.md を参照）:
1. 要件定義（EARS記法）
2. データ定義（8_data/ に RON ファイル）
3. 仕様書作成:
...
```
人間: 「ジャンプ機能の実装タスクを作成して」

メイン Claude Code の動作（task-registration.md を参照）:
1. 仕様書を参照
2. ID採番（/id-next → 30101）
3. タスクファイル作成（Write）:
...
### クイックスタート B: バグ修正/フレームワーク（プランモード経由）

**目標**: 既存機能のバグを修正する、またはフレームワークを更新する

#### 1. プラン作成

```
人間: 「ジャンプの着地判定バグを修正したい」

メイン Claude Code の動作:
1. プランモードに移行
2. 関連調査（Explore agent）
3. ユーザー確認（必要に応じて）
4. プランファイル作成:
   ~/.claude/plans/task-landing-bug.md
5. プランモード終了
```
...
1. プランファイル検出
2. タスクタイプ判定（bugfix）
3. ID採番（/id-next → B30101-001）
4. タスクファイル作成:
   project/tasks/1_todo/B30101-001-着地判定バグ修正.md
```
...
タスク作成後は、タスクタイプに関わらず以下のフローで進めます。
...
→ worktree作成（game-dev/bugfix/refactorタスクの場合）
```
...
**責務分離:**
- **task-registration.md**: プランファイル → タスクファイル変換
- **task-manager-agent**: タスクライフサイクル管理（開始/完了/worktree）

**タスク登録（task-registration.md を参照）:**
```
「プランからタスクを作成して」
...
| 「プランからタスクを作成」「タスク登録」 | 📝 task-registration.md |
