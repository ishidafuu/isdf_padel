...
> **NOTE**: 簡素化ステップは実装タスク（30XXX/B30XXX/R30XXX）で変更10行超の場合に実行。
...
### 簡素化ステップ（in-review 遷移前）

実装タスク（30XXX/B30XXX/R30XXX）では、`in-review` 遷移前に `code-simplifier` を実行。
...
| FXXX/PXXX タスク | ドキュメント変更のため対象外 |

詳細: `agents/impl-agent.md`（簡素化ステップセクション）
