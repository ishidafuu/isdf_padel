...
| `/run-game [--release]` | ゲームを起動 | 動作確認時 |
| `/delete-replays` | リプレイファイルを削除 | 不要リプレイの削除時 |
...
| コマンド | 使用エージェント | 自動実行タイミング | 説明 |
|---------|----------------|------------------|------|
| `/code-audit` | audit-agent | 定期監査時 | プロジェクト全体のコード健康診断 |

**例（audit-agentが自動実行）**:
```bash
# audit-agent がフル監査を実行
/code-audit --full
→ === Code Audit Report ===
→ [CRITICAL] 循環依存: player → ball → court → player
→ [MAJOR] @spec コメント欠如: project/src/game/player.rs:45

# 差分監査（変更ファイルのみ）
/code-audit --diff
→ 2 files changed, 1 issue found
```
