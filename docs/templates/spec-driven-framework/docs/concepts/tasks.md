...
**ポイント:**
- **ゲーム開発**: 仕様書自体が「計画」の役割を果たすため、プランモードは不要
- **その他**: 既存コードの修正・改善計画を立ててからタスク化

---

## タスクタイプ

5つのタスクタイプがあり、対象範囲と worktree 対応が異なります。

| タスク種別 | ID形式 | 配置場所 | worktree | 対象範囲 |
|-----------|-------|---------|---------|---------|
| **ゲーム開発** | `30XXX` | `project/tasks/` | 有効 | `project/` 配下の仕様書・実装 |
| **バグ修正** | `B30XXX-NNN` | `project/tasks/` | 有効 | 既存機能のバグ修正 |
| **リファクタ** | `R30XXX-NNN` | `project/tasks/` | 有効 | 既存機能の改善・最適化 |
...
**A:** タスクタイプによってタイミングが異なります。

**ゲーム開発（30XXX）の場合:**
```
1. requirements-agent → spec-agent → critic-agent（仕様策定）
2. design-agent → behavior-agent（詳細設計）
3. 仕様書コミット
4. タスク作成（仕様書から実装タスクを抽出）
5. impl-agent（実装）
```

**その他（バグ修正・リファクタ・フレームワーク・プロジェクト横断）の場合:**
```
1. プランモードで計画作成
2. タスク作成（プランからタスクファイル生成）
3. 実装
```
