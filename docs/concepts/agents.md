...
| 依存関係を確認 | 🔗 deps-agent |
| プランからタスクを作成 | 📝 task-registration-agent |
...
└─────────────────────────────────────────┘
           │
           ├─ 0. プランからタスクを作成 → 📝 task-registration-agent
...
**いつ使う:** 新規プロジェクト開始時（1回のみ）

**やること:**
1. ミニマム構成のフォルダ作成
2. CLAUDE.md 生成
3. 10001_concept.md 作成
4. .claudeignore 作成
5. エージェントファイル配置
...
**いつ使う:**
- 新規プロジェクト開始時（Phase 0.5）: setup-agent の直後
...
### 🔬 legacy-analyzer-agent（レガシーコード解析）

**いつ使う:** 既存のレガシーコード（C++等）から仕様を抽出したいとき

**やること:**
1. 段階的解析（粗視化 → 詳細 → アルゴリズム抽出）
2. 制作者の記憶との協働による仕様復元
3. 9_reference/901_reference_game/ への記録

**入力:** legacy/ 配下のソースコード、制作者の記憶
**出力:** 9_reference/901_reference_game/ への仕様ドキュメント
...
### 🎮 game-reference-agent（参照ゲーム仕様記録）

**いつ使う:** 既存ゲームを参考資料として記録したいとき

**やること:**
1. 画像・説明・Web情報からの仕様抽出
2. UI/UX 要素の分析
3. ゲームメカニクスの観察と言語化
4. 共通パターンの抽出

**入力:** スクリーンショット、ゲーム説明、Web 情報
**出力:** 9_reference/901_reference_game/ への仕様ドキュメント

**重要**: マルチモーダル対応。画像を直接分析可能。
...
### legacy-analyzer-agent vs game-reference-agent

| 状況 | 使用エージェント | 入力 |
|------|----------------|------|
| **過去に自分が作ったゲームのコードがある** | 🔬 legacy-analyzer-agent | legacy/ 配下のソースコード |
| **他のゲームを参考にしたい** | 🎮 game-reference-agent | 画像・説明・Web情報 |
| **コードはないが、動画や攻略サイトがある** | 🎮 game-reference-agent | Web情報・スクリーンショット |
| **C++/その他の大規模コードを解析したい** | 🔬 legacy-analyzer-agent | Serena MCP経由で解析 |

**共通点**: どちらも 9_reference/ に参照資料を配置し、spec-agent が新規仕様作成時に参照。
